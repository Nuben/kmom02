...
function get_debug() {
  ...
  if(isset($ly->config['debug']['Chris']) && $ly->config['debug']['Chris']) {
    $html .= "<hr><h3>Debuginformation</h3><p>The content of CChris:</p><pre>" . htmlent(print_r($ly, true)) . "</pre>";
  }
  if(isset($ly->config['debug']['session']) && $ly->config['debug']['session']) {
    $html .= "<hr><h3>SESSION</h3><p>The content of CChris->session:</p><pre>" . htmlent(print_r($ly->session, true)) . "</pre>";
...
function get_messages_from_session() {
  $messages = CChris::Instance()->session->GetMessages();
  $html = null;
  if(!empty($messages)) {
...
function render_views() {
  return CChris::Instance()->views->Render();
}